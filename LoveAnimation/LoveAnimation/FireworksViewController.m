...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    for (id layer in self.view.layer.sublayers) {
        if([layer isKindOfClass:[RPAnimationLayer class]])
        {
            [layer removeFromSuperlayer];
        }
    }
    [RPAnimationLayer createAnimationLayerWithString:@"梦，好喜欢你吖！" andRect: CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.width) andView:self.view andFont:[UIFont boldSystemFontOfSize:30] andStrokeColor:[UIColor cyanColor]];
}
